...
dhr2 <- dhr %>%
  ...
  mutate(PID = as.character(PID),
         ...
         craniectomy_yn = factor(case_when(DHBPR == 1 ~ "Yes",
                                           TRUE ~ "No")))
#         hemotoma_surg_any = as.factor(case_when(DHSHE == 1 | DHEH == 1 | DHPHE == 1 ~ "Surg", 
#                                                 DHSHE != 1 & DHEH != 1 & DHPHE != 1 ~ "None", 
#                                                 TRUE ~ NA_character_)))
#   hemotoma variables, decompressive craniectomy var not useful per se - too many NAs 



#mutate(DHPBLAT = as.factor(DHPBLAT), DHPASYM = as.factor(DHPASYM), 
  #       DHMECHV = as.factor(DHMECHV),
  #       dhr_hypotension = case_when(DHSBPL >= 90 ~ "0",
  #                                   DHSBPL < 90 ~ "1",
  #                                   TRUE ~ NA_character_) %>% as.factor,
  #       dhr_hypoxia = case_when(DHLRPAO >= 50 ~ "0",
  #                               DHLRPAO < 50 ~ "1",
  #                               TRUE ~ NA_character_) %>% as.factor,
  #       pupil_dhr = case_when(DHPASYM == "1" | DHPBLAT == "1" ~ "0",
  #                                 DHPASYM == "2" & DHPBLAT == "2" ~ "1",
  #                                 TRUE ~ NA_character_))
...
#Pre hosp APBILAT, APASYM, pupil_prehosp - NS
   ...
   #chisq.test(factor(tbi_clean$bilat_prehosp), tbi_clean$outcome, correct = F)
   #chisq.test(factor(tbi_clean$asymm_prehosp), tbi_clean$outcome, correct = F)
...
summary_table2 <-
  list(
    ...
    "Pre-Hospital Pupil Abnormalities (Y/N) - n(%)" =
      list("Yes" = ~ n_perc0(.data$pupil_prehosp == "Yes", na_rm = T),
           "No"  = ~ n_perc0(.data$pupil_prehosp == "No", na_rm = T))
  )
...
summary_table3 <-
  list(
    ...
    "Abnormal Pupils in ED - n(%)" = 
      list("Yes" = ~ n_perc0(.data$pupil_ed == "Yes", na_rm = T),
           "No" =  ~ n_perc0(.data$pupil_ed == "No", na_rm = T)),
...
rep_rows <- c(1, 3, 6, 8, 10, 12, 14, 16)
